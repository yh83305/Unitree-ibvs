...
class AprilTag:
    def __init__(self):
        # 相机参数 (需要根据实际相机进行校准)
        self.fx = 924.2759399414062  # 焦距 x
        self.fy = 924.2759399414062  # 焦距 y
        self.cx = 640  # 主点 x
        self.cy = 360  # 主点 y
        
        # 期望 AprilTag 角点坐标 (单位: 像素)
        self.desired_corners = np.array([
            [570, 290], [670, 290], [670, 390], [570, 390],
        ], dtype=np.float32)
        ...
        self.depth_subscriber = rospy.Subscriber('/d435/depth/image_raw', Image, self.depth_callback)
    ...
    def detect_apriltag(self, color_image):
        ...
        for tag in tags:
            ...
            for (i, (ud, vd)) in enumerate(self.desired_corners):
                cv2.circle(color_image, (int(ud), int(vd)), 5, (255, 0, 255), -1)
                cv2.putText(color_image, str(i), (int(ud) + 5, int(vd) - 5), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
